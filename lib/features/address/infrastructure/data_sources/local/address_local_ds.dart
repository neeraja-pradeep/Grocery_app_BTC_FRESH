...
class AddressLocalDs {
  ...
  Future<CachedAddressListResult?> getCachedAddresses() async {
    ...
    try {
      ...
      final addressDtos = addressListJson
          .map((json) =>
              AddressCacheDto.fromJson(Map<String, dynamic>.from(json as Map)))
      ...
      return CachedAddressListResult(
  ...
  Future<void> _saveCacheInternal(List<AddressDto> addresses) async {
    try {
      final cacheDtos =
          addresses.map(AddressCacheDto.fromDto).map((dto) => dto.toJson()).toList();
